...
const ArtCard: React.FC<ArtCardProps> = ({
    ...
    return (
        <div
            className="relative flex flex-col rounded-lg overflow-hidden shadow-lg bg-[#094129] text-white transition-all duration-300 w-full h-full"
        ...
        >
            ...
            {/* Art Details Section */}
            <div
                className={`flex flex-col justify-around transition-all duration-300 ${isExpanded ? 'h-1/2' : 'h-1/4'
                    }`}
            >
                {/* Basic Info */}
                <div className="px-2">
                    ...
                    {isExpanded && (
                        <div className="space-y-1 text-[2vmin] text-white">
                            <p className='pt-1'><strong>Artist:</strong> {artistName}</p>
                            <p><strong>Framed:</strong> {framed}</p>
                            <p><strong>Orientation:</strong> {orientation}</p>
                            <p className='pb-1'><strong>Style:</strong> {artStyle}</p>
                            <hr className='my-1 border-gray-500'></hr>
                        </div>
